...
const { inferMarket, isBoardIndex } = require('./commands/helpers');
...
async function runScan(ctx, cache, items, criterionNames, opts = {}) {
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    // M1-2: 板块指数（88xxxx）不可作为个股选股
    if (isBoardIndex(item.code)) {
      Object.assign(rec, { passed: false, skipped: true, error: '板块指数（88xxxx）不可选股，仅供观察强弱' });
      results.push(rec);
      continue;
    }
    try {
      ...
      // M2-2/方向门控: --only-hot 时大盘普跌（环境冷）→ 抑制做多类命中（"不逆大盘"落地到选股）
      let gateNote = null;
      if (passed && opts.marketCold) {
        passed = false;
        gateNote = '大盘普跌(环境冷)，--only-hot 抑制做多';
      }

      Object.assign(rec, { analysis, patterns, score, matched, passed, skipped: false, gateNote });
    } catch (e) {
