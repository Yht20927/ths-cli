...
const { getFlag, renderTable, fmtNum, inferMarket, PERIODS, formatQuotes } = require('./helpers');
...
async function dailyRun(ctx, args) {
  ...
  for (const pos of ctx.cache.positionsList()) {
    ...
    if (snap && snap.signal === '看空') {
      ...
      reminders.push(`${pos.code} ${snap.name || pos.code}: 收盘跌破支撑 ${snap.support}，止损纪律触发`);
    }
  }
  ...
  if (args.includes('--json')) {
    return { date: runDate, marketEnv, stocks: snapshots, stats, suggestions: openSugs, filled, warnings: warns, holdings, reminders };
  }

  const changes = signalChanges(store);
  renderRunReport({ runDate, marketEnv, snapshots, stats, suggestions: openSugs, filled, warns, minN, changes, reminders, holdings, sinceDays });
  return undefined;
}

function renderRunReport(r) {
  const { runDate, marketEnv, snapshots, stats, suggestions, filled, warns, minN, changes, reminders, holdings, sinceDays } = r;
  ...
  if (!snapshots.length) {
    ...
    console.log('（命中列 = 该股历史该信号命中率，如 62%(13) 为命中率62%、样本13）');
  }
  ...
  if (holdings && holdings.length) {
    ...
    console.log(renderTable(holdings.map(h => ({
      ...
      signal: h.signal || '-',
      support: h.support != null ? r2(h.support) : '-',
    })), [
      { header: '代码', key: 'code' },
      { header: '名称', key: 'name' },
...
function printBucketTable(stats, minN) {
  ...
  const label = { signal: '信号', scoreBand: '评分', ma: '均线', marketMood: '市场', macdDir: 'MACD', kdjRegime: 'KDJ', rsiRegime: 'RSI6', adxRegime: 'ADX', atrRegime: 'ATR%', patternDir: '形态', resonance: '共振' };
  const rows = [];
...
async function dailySnapshot(ctx, args) {
  ...
  if (args.includes('--json')) return { date, marketEnv: file.marketEnv, stocks: rows };
  ...
  for (const s of rows) {
    ...
    const outStr = o
      ...
      : '';
    console.log(`  ${s.code} ${s.name || ''} 收盘 ${s.close}  评分 ${s.score}/${s.signal}  均线 ${s.maAlignment}  MACD ${s.macdStatus}  KDJ ${s.kdj ? s.kdj.k + '/' + s.kdj.d : '-'}  RSI6 ${s.rsi6}  支撑 ${s.support} 压力 ${s.resistance}${outStr}`);
