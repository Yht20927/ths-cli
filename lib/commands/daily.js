...
const { fetchAllIndexBars, summarizeIndexTrend } = require('../index-kline');
...
async function dailyRun(ctx, args) {
  ...
  // ①a 大盘指数趋势（M2-2: Node 直连指数日K → 上证/创业板 是否站上 MA20 等）
  {
    try {
      const all = await fetchAllIndexBars();
      const summ = {};
      for (const code of ['1A0001', '399006']) {
        const bars = all[code];
        if (bars && bars.length) summ[code] = summarizeIndexTrend(bars);
      }
      if (Object.keys(summ).length) marketEnv.indices = summ;
    } catch (e) {
      warns.push(`大盘指数趋势获取失败: ${e.message}`);
    }
  }
...
function renderRunReport(r) {
  ...
  // M2-2: 大盘指数趋势（上证/创业板 MA20 位置）
  if (marketEnv && marketEnv.indices) {
    const parts = [];
    for (const code of ['1A0001', '399006']) {
      const t = marketEnv.indices[code];
      if (!t) continue;
      const label = code === '1A0001' ? '上证' : '创业板';
      const gap = t.maGapPct != null ? `${t.maGapPct >= 0 ? '+' : ''}${t.maGapPct}%` : '-';
      parts.push(`${label} ${t.close} ${t.aboveMA20 ? '站上MA20' : '跌破MA20'}(${gap}) ${t.maAlignment === '多头排列' ? '多' : t.maAlignment === '空头排列' ? '空' : ''}`);
    }
    if (parts.length) console.log(`指数趋势: ${parts.join('  |  ')}`);
  }
  // M2-1: 方向环境（板块/资金/龙虎榜）
