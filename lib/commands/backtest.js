...
async function cmdBacktest(ctx, args) {
  const code = args[0];
  if (!code) throw new Error('用法: ths backtest <code> [--strategy ma-cross|rsi|macd|score|resonance|buy-hold] [--fast N] [--slow N] [--score-buy 60] [--score-sell 40] [--score-min 60] [--adx 25] [--fee 0.0005] [--include-forming] [--json]');
  ...
  if (strategy === 'ma-cross') {
    ...
    params.buy = clampScore(parseInt(getFlag(args, '--score-buy', 60), 10) || 60);
    params.sell = clampScore(parseInt(getFlag(args, '--score-sell', 40), 10) || 40);
  } else if (strategy === 'resonance') {
    params.scoreBuy = clampScore(parseInt(getFlag(args, '--score-min', 60), 10) || 60);
    params.adxMin = Math.max(0, parseInt(getFlag(args, '--adx', 25), 10) || 25);
  }
  ...
  // 数据卫生：回测是 close-to-close 历史引擎，绝不该把盘中"在途半截 bar"当已收盘历史。
  // 默认剔除在途末根（loadKline excludeForming）；--include-forming 显式包含（逃生口，
  // 覆盖收盘后 15:00–15:05 K 线定型缓冲窗口内的个别场景）。
  const includeForming = args.includes('--include-forming');
  ...
  console.log(renderKV([
    ...
    { label: '在途K线', value: includeForming ? '包含当日未收盘（--include-forming）' : '剔除当日未收盘（默认，回测用已定型bar）' },
  ]));
  console.log('');
