...
/**
 * 是否同花顺板块指数（88xxxx，如 881129 通信设备 / 886100 概念）。
 * 板块指数是"强弱观察"标的，不可直接买卖——防止快照出"看多"误导当个股买。
 * @param {string|number} code
 * @returns {boolean}
 */
function isBoardIndex(code) {
  return /^88\d{4}$/.test(String(code));
}
...
// 实测字段：6价/7开/8高/9低/10昨收/13量(股)/19额(元)/69涨停/70跌停/
// 199112涨跌幅/264648涨跌额/3153PE/592920PB/3475914总市值/3541450流通市值/
// 1968584换手率/1771976量比
const QUOTE_FIELDS = {
  '6': 'price',          // 最新价
  '7': 'open',           // 开盘
  '8': 'high',           // 最高
  ...
  '3153': 'pe',          // 市盈率(动)
  '592920': 'pb',        // 市净率
  '3475914': 'totalMv',  // 总市值(元)
  '3541450': 'floatMv',  // 流通市值(元)
  '1968584': 'turnoverRate', // 换手率(%)
  '1771976': 'volumeRatio',  // 量比
};

/**
 * 实时行情快照解析：按响应 data_fields 位置映射
 * @param {object} qd - quote_data[0]
 * @returns {object|null} {code, market, price, open, high, low, prevClose, volume, amount, pct, change, ...}
 */
...
/**
 * 批量行情解析：quote_data 数组 → 结构化数组
 * @param {Array<object>} qdList
 * @returns {Array<object>}
 */
function formatQuotes(qdList) {
  return (qdList || []).map(formatQuote).filter(Boolean);
}
