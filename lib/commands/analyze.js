...
const { renderKV, formatQuote, escapeExpression, fmtNum, isBoardIndex } = require('./helpers');
const {
...
async function cmdAnalyze(ctx, args) {
  ...
  // ── 股票名称（search 缓存解析，失败不阻塞）──
  const name = (await resolveName(ctx, ctx.cache, params.code)) || null;

  // ── 板块指数标记（M1-2: 881xxx 不可直接买入）──
  const isBoard = isBoardIndex(params.code);

  // ── 估值/换手（实时快照，失败不阻塞）──
  let valuation = null;
  try {
    const qd = await ctx.loggedCall('quote', { code: params.code, market: params.market },
      `window.__ths.quote('${escapeExpression(params.code)}', '${escapeExpression(params.market)}')`);
    const f = formatQuote(qd);
    if (f) {
      valuation = {
        price: f.price, pct: f.pct, change: f.change,
        turnoverRate: f.turnoverRate, volumeRatio: f.volumeRatio,
        pe: f.pe, pb: f.pb, totalMv: f.totalMv, floatMv: f.floatMv,
        limitUp: f.limitUp, limitDown: f.limitDown,
      };
    }
  } catch (e) { /* 估值获取失败不影响分析 */ }
  ...
  // M1-3: 信号分级（共振升级/矛盾降级）
  const sigInfo = classifySignal(score, {
    maAlignment: a.ma.alignment,
    macdStatus: a.macd.status,
    adx: adxR.adx[last],
    patterns: recentPatterns(patterns, 3),
    kdj: { k: a.kdj.k, d: a.kdj.d },
    rsi6: a.rsi.rsi6,
  ...
  if (args.includes('--compact')) {
    const s = buildSummary(code, bars, { analysis: a, patterns, score, sr });
    const val = valuation;
    const parts = [
      `评分 ${s.score} ${s.signal}`,
      s.maAlignment,
      ...
      `支撑 ${s.support ?? '-'}`,
      `压力 ${s.resistance ?? '-'}`,
      s.patterns.length ? `形态 ${s.patterns.join('/')}` : null,
      val ? `换手 ${val.turnoverRate ?? '-'}% 量比 ${val.volumeRatio ?? '-'} PE ${val.pe ?? '-'} PB ${val.pb ?? '-'}` : null,
    ].filter(Boolean);
    console.log(`${code}${name ? ' ' + name : ''}  收盘 ${s.close}  ${parts.join('  |  ')}`);
    if (isBoard) console.log('⚠ 板块指数（88xxxx）——不可直接买入，仅供观察板块强弱');
    return undefined;
  }

  console.log(`════ ${code}${name ? ' ' + name : ''} ${period} K线分析（${a.count} 根，${a.firstDate} ~ ${a.lastDate}）════`);
  if (isBoard) console.log('⚠ 板块指数（88xxxx）——不可直接买入，仅供观察板块强弱');
  ...
  if (valuation) {
    ...
    console.log(renderKV([
      { label: '现价/涨跌', value: `${valuation.price}（${sign(valuation.pct)}%）` },
      { label: '换手率', value: `${valuation.turnoverRate != null ? valuation.turnoverRate + '%' : '-'}` },
      { label: '量比', value: valuation.volumeRatio != null ? valuation.volumeRatio : '-' },
      { label: '市盈率/市净率', value: `${valuation.pe != null ? valuation.pe : '-'} / ${valuation.pb != null ? valuation.pb : '-'}` },
      { label: '总市值', value: valuation.totalMv != null ? fmtNum(valuation.totalMv) : '-' },
      { label: '涨停/跌停', value: `${valuation.limitUp != null ? valuation.limitUp : '-'} / ${valuation.limitDown != null ? valuation.limitDown : '-'}` },
    ]));
  }
  ...
  console.log('\n综合评分:');
  console.log(renderKV([
    { label: '评分', value: `${score.total}/100（${sigInfo.label}）` },
    { label: '信号冲突', value: sigInfo.conflicts.length ? sigInfo.conflicts.join(' / ') : '无（共振一致）' },
