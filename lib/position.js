...
/**
 * 计算止损/目标/盈亏比（不含仓位），供 calcPosition 与 portfolio 共用。
 * @param {Array<{open,high,low,close}>} bars
 * @param {object} opts - { price, atrMult=2, stop, target }
 * @returns {object} { price, atrValue, atrPct, stopPrice, stopSource, stopDistPct,
 *                     targetPrice, targetSource, upsidePct, riskReward, rrGrade, riskPerShare }
 */
function calcStopTarget(bars, opts = {}) {
  const price = opts.price != null ? Number(opts.price) : bars[bars.length - 1].close;
  const atrMult = opts.atrMult != null ? Number(opts.atrMult) : 2;
  const highs = bars.map(b => b.high);
  const lows = bars.map(b => b.low);
  const closes = bars.map(b => b.close);
  const atrS = atr(highs, lows, closes, 14);
  const atrV = atrS[bars.length - 1] || 0;
  const atrPct = atrV > 0 && price > 0 ? (atrV / price) * 100 : null;
  ...
  const rrGrade = riskReward != null
    ...
    : null;
  const stopDistPct = riskPerShare > 0 ? (riskPerShare / price) * 100 : 0;

  return {
    price, atrValue: atrV, atrPct, stopPrice, stopSource, stopDistPct,
    targetPrice, targetSource, upsidePct, riskReward, rrGrade, riskPerShare,
  };
}
...
function calcPosition(bars, opts = {}) {
  ...
  const st = calcStopTarget(bars, { price, atrMult, stop: opts.stop, target: opts.target });
  if (!(st.atrValue > 0)) throw new Error('ATR 计算失败（数据太短？）');

  const {
    atrValue: atrV, atrPct, stopPrice, stopSource, stopDistPct,
    targetPrice, targetSource, upsidePct, riskReward, rrGrade, riskPerShare,
  } = st;
...
module.exports = { calcPosition, calcStopTarget };
