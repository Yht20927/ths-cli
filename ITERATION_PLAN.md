...
> Slice 1 = 实时能力落地（本表）；**Slice 2 组合/风控层量化 `ths risk`**、**Slice 3 因子/选股广度量化（打分/共振进回测 + 方向门控）** 为候选下一步。
...
*2026-09-03 实现：M4-1 盘中实时追踪 `ths watch` + M4-2 半截 K 线数据卫生（Slice 1；测试全量 300 绿 + 盘中实盘验证）；克制清单第 4 条改写为限定版。下一步候选：M2-2 大盘趋势 / Slice 2 组合风控 `ths risk`。*
